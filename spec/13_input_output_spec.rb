...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        negative = '-9'
        valid_input = '5'
        allow(game_loop).to receive(:player_input).and_return(letter, negative, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_four_plus) { described_class.new(7, '7', 5) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        winning_phrase = "That was hard. It took you 5 guesses!\n"
        expect { game_four_plus.final_message }.to output(winning_phrase).to_stdout
      end
    end
  end
end
