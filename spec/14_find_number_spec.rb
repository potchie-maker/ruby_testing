...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are not equal' do
      let(:number_not_over) { double('random_number', value: 4) }
      subject(:game_not_over) { described_class.new(0, 9, number_not_over, 8) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:edge_range) { described_class.new(5, 8, number_range, 7) }

      before do
        edge_range.update_range
      end

      it 'updates min to the same value as max' do
        expect(edge_range.min).to eq(8)
      end

      it 'does not update max' do
        expect(edge_range.max).to eq(8)
      end
    end
  end
end
